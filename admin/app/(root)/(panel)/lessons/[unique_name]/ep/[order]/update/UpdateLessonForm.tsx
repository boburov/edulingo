...
export default function UpdateLessonForm() {
  ...
  const { playlist, setPlaylist } = useContext<{
    playlist: Playlist;
    setPlaylist: any;
  }>(GlobalContext);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const i = playlist.lessons.findIndex(
        (l: Lesson) => l.id === res_lesson.id
      );
      ...
      setPlaylist((prev: Playlist) => ({
        ...
        lessons: prev.lessons.map((l) =>
          l.id === res_lesson.id ? res_lesson : l
        ),
      }));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="space-y-3">
        <h2 className="text-xl font-semibold">Tahrirlash</h2>
      </div>
